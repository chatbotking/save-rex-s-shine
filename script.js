...
(function(){
  ...
  function run(){
    if(contactId){
      ...
      xhr.open('GET','https://paymegpt.com/api/landing/context/'+encodeURIComponent(contactId)+'?page_id=2353');
      xhr.onload=function(){
        if(xhr.status===200){
          try{
...
(function(){
  ...
  window.__donationConfig={currency:currency,presets:[5,10,25,50],minAmount:100/100,maxAmount:1000000/100,formatAmount:fmtD,buttonColor:'#36D6B5',buttonText:'Keep Rex Shining 💸',thankYouMessage:'You\'re a real one. Rex sees you. Rex appreciates you. Rex will think about you next time he\'s cruising down Sunset. 🐾🕶️'};
  ...
  document.addEventListener('DOMContentLoaded',function(){
    ...
    if(urlParams.get('donation')==='success'){
      ...
      overlay.style.cssText='position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center;z-index:999999;font-family:system-ui,-apple-system,sans-serif;';
      overlay.innerHTML='<div style="background:white;border-radius:16px;padding:40px;max-width:420px;width:90%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.15);"><div style="width:64px;height:64px;border-radius:50%;background:#dcfce7;margin:0 auto 20px;display:flex;align-items:center;justify-content:center;"><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#16a34a" stroke-width="3"><polyline points="20 6 9 17 4 12"/></svg></div><h2 style="margin:0 0 12px;font-size:24px;font-weight:700;color:#111827;">Thank You!</h2><p style="margin:0 0 24px;color:#6b7280;font-size:16px;">You\'re a real one. Rex sees you. Rex appreciates you. Rex will think about you next time he\'s cruising down Sunset. 🐾🕶️</p><button onclick="document.getElementById(\'donation-success-overlay\').remove();window.history.replaceState({},\'\',window.location.pathname);" style="padding:12px 32px;font-size:16px;font-weight:600;background:#16a34a;color:white;border:none;border-radius:8px;cursor:pointer;">Continue</button></div>';
      document.body.appendChild(overlay);
    }
  });
})();

// Initialization and Donation Handling
        document.addEventListener('DOMContentLoaded', () => {
            // Use window.__donationConfig or fallback for development
            const config = window.__donationConfig || {
                currency: 'USD',
                presets: [5, 10, 25, 50],
                minAmount: 1,
                maxAmount: 10000,
            ...
            let selectedAmountCents = 2500; // Default to $25 (2500 cents)

            // Create buttons from config
            config.presets.forEach((dollars, index) => {
                const btn = document.createElement('button');
                btn.type = 'button';
                btn.className = `preset-btn p-4 rounded-xl border-2 transition-all font-display text-2xl flex flex-col items-center justify-center ${index === 2 ? 'border-rexTeal bg-rexTeal/10 text-rexTeal' : 'border-white/10 text-gray-400 hover:border-white/30'}`;
                btn.innerHTML = `
                    <span class="text-xs font-sans font-bold opacity-60 mb-1">SUPPORT</span>
                    $${dollars}
                `;
                btn.onclick = () => {
                    // Update selection
                    document.querySelectorAll('.preset-btn').forEach(b => {
                        b.classList.remove('border-rexTeal', 'bg-rexTeal/10', 'text-rexTeal');
                        b.classList.add('border-white/10', 'text-gray-400');
                    });
                    btn.classList.add('border-rexTeal', 'bg-rexTeal/10', 'text-rexTeal');
                    btn.classList.remove('border-white/10', 'text-gray-400');
                    selectedAmountCents = dollars * 100;
                };
            ...
            document.getElementById('final-submit').onclick = () => {
                ...
                if (!email) {
                    alert('Rex needs your email to send a digital "thank you" bark!');
                    return;
                }

                if (typeof window.__processDonation === 'function') {
                    window.__processDonation(selectedAmountCents, email, name, message);
                } else {
                    ...
                    alert('Donation system integration active. Rex is wagging his digital tail! (This is a demo environment)');
                }
            };
        });
...
window.PayMeGPTConfig = {
        ...
        stopButtonText: 'Stop (Rex is crying)',
        theme: 'dark',
